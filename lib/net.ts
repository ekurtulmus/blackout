...
export class NetRoom {
  ...
  onStart: (payload: { diff: string; order: string[]; names: string[]; themeSeed: number; level: unknown; pvp?: boolean; arena?: boolean }) => void =
    () => {};
  ...
  connect() {
    ...
    ch.on("broadcast", { event: "start" }, (p) => {
      const d = p.payload as { diff: string; order: string[]; names: string[]; themeSeed: number; level: unknown; pvp?: boolean; arena?: boolean };
      if (!d || this.role === "host") return;
      this.started = true;
      this.stopTimers();
      this.onStart(d);
  ...
  startGame(payload: { diff: string; level: unknown; themeSeed: number; pvp?: boolean; arena?: boolean }) {
    if (this.role !== "host") return;
    this.started = true;
    this.stopTimers();
    const players = this.getPlayers();
